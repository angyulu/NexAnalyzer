...
def _frames_table(frames):
    return pd.DataFrame(
        ...
        columns=[
            "Class", "Position", "N_Frames", "N_Noise_Limited",
...
class TestFrameClassStats:
    ...
    def test_noise_limited_frames_are_counted_per_class(self):
        """A noise-limited side's coverage is a lower bound. The class table
        cannot exclude it without disagreeing with the points table, so it
        counts it, per class, next to N_Frames."""
        frames = [
            _frame(1, (2.0, 96.0, 2.0)),
            _frame(2, (2.0, 96.0, 2.0), threshold_mode="midpoint",
                   status_below="midpoint", status_above="noise-limited"),
            _frame(3, (2.0, 96.0, 2.0), threshold_mode="midpoint",
                   status_below="noise-limited", status_above="noise-limited"),
        ]
        df = _frames_table(frames).set_index("Position")

        assert df.loc["Below", "N_Noise_Limited"] == 1
        assert df.loc["Above", "N_Noise_Limited"] == 2
        # The reference row counts frames where either side is a bound.
        assert df.loc["Reference", "N_Noise_Limited"] == 2

    def test_a_plain_adaptive_run_counts_no_noise_limited_frames(self):
        df = _frames_table([_frame(1, (1.0, 97.0, 2.0))])

        assert (df["N_Noise_Limited"] == 0).all()


# ------------------------------------------------------------- OM point table
class TestFramePointRows:
    ...
    def test_how_the_cuts_were_placed_travels_with_the_row(self):
        frame = _frame(4, (1.0, 96.0, 3.0), threshold_mode="midpoint",
                       step_below=9.1, step_above=6.3,
                       status_below="midpoint", status_above="noise-limited")
        row = self._points([frame]).iloc[0]

        assert row["Threshold_Mode"] == "midpoint"
        assert row["Step_Below_pct"] == pytest.approx(9.1)
        assert row["Step_Above_pct"] == pytest.approx(6.3)
        assert row["Status_Below"] == "midpoint"
        assert row["Status_Above"] == "noise-limited"

    def test_an_adaptive_row_says_so_and_has_no_step(self):
        row = self._points([_frame(1, (1.0, 97.0, 2.0))]).iloc[0]

        assert row["Threshold_Mode"] == "adaptive"
        assert row["Status_Above"] == "adaptive"
        assert pd.isna(row["Step_Above_pct"])
