...
- **Current implementation:** Not possible, must fit over entire range
...
### Implementation Status (as of v2.9.0)

Solutions 1-9 (§6.1, §6.2) were implemented as manual-UI conveniences in
...
| Solution | Algorithm-level status | UI-level status |
|---|---|---|
| 1-2. Lambda slider range/scale | N/A (no slider) | ⬜ Removed with the manual UI |
...
In short: the baseline *algorithms* are largely untouched and still do
everything this document describes; what's gone is the manual-tuning
*interface* to them. Two items are worth a second look if manual UI ever
returns or presets are extended: item 8 (Rolling Ball/Spline/airPLS are
implemented but unreachable from a preset today) and item 9 (quality
metrics are computed nowhere in the current pipeline).
