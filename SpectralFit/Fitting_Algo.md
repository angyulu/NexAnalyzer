# SpectralFit Voigt Fitting Algorithm Documentation

**Author:** Claude Code
**Date:** 2025-12-21
**Purpose:** Comprehensive documentation of the Voigt peak fitting algorithm, including fundamental logic, workflow, current implementation issues, and optimization proposals.
...
### 6.1 Fix Amplitude Initialization
...
### 6.2 Shape-Aware Width Initialization
...
### 6.3 Improved Auto-Find FWHM Estimation
...
1. **Fix amplitude initialization** (Section 6.1)
   - **Effort**: 30 minutes
   - **Impact**: High (solves most "poor fit quality" issues)
   - **Files**: `fitting.py:119-120`

2. **Implement adaptive bounds** (Section 6.4)
   - **Effort**: 1 hour
   - **Impact**: High (prevents bound-hitting issues)
   - **Files**: `peak.py:106-126`

3. **Improve auto-find FWHM** (Section 6.3)
   - **Effort**: 1 hour
   - **Impact**: Medium (better auto-find quality)
   - **Files**: `fitting.py:346-353`

### 7.2 Medium-Term Improvements (Medium Priority)

4. **Shape-aware width initialization** (Section 6.2)
   - **Effort**: 1 hour
   - **Impact**: Medium (better shape fitting)
   - **Files**: `fitting.py:113-114`

5. **Robust error handling** (Section 6.6)
   - **Effort**: 30 minutes
   - **Impact**: Low (code quality)
   - **Files**: `fitting.py:138-153`

6. **Peak overlap detection** (Section 6.7)
   - **Effort**: 1 hour
   - **Impact**: Medium (user experience)
   - **Files**: `fitting.py` (new function), `control_panel.py` (warning display)
...
10. **Unit tests for fitting functions**
    - **Effort**: 4 hours
    - **Coverage**: Synthetic data with known parameters
    - **Files**: `tests/test_fitting.py`

11. **Integration tests for UI workflow**
    - **Effort**: 2 hours
    - **Coverage**: Auto-find → Fit → Export
    - **Files**: `tests/test_workflow.py`

12. **Benchmark test suite**
    - **Effort**: 4 hours
    - **Coverage**: Real Raman/PL spectra with ground truth
...
1. ❌ **Amplitude mismatch** (height vs. integrated intensity) → Poor convergence
2. ❌ **Width initialization bias** (assumes equal Gaussian/Lorentzian) → Wrong shapes
3. ❌ **Bounds too tight** (center ± 5 cm⁻¹ for Raman) → Cannot escape local minima
4. ⚠️ **Single-stage fitting** → Prone to local minima with many peaks

### Recommended Action Plan:
1. **Immediate**: Implement fixes 1-3 from Section 7.1 (3 hours total)
2. **Test**: Create synthetic data with known parameters, verify R² > 0.99
3. **User feedback**: Deploy to production, collect real-world fit examples
4. **Iterate**: Implement medium-term improvements based on feedback
5. **Long-term**: Consider multi-stage fitting if quality issues persist
