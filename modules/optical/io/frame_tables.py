...
Both tables also say how the cuts were placed. A frame whose side is
`noise-limited` (midpoint mode found a population whose half-step sits inside
the film's noise) reports a coverage that is a lower bound, not a measurement;
the class table counts those frames per class so the mean is read with that in
mind rather than silently including them.
"""
...
POINT_COLUMNS: Tuple[str, ...] = (
    ...
    "Threshold_Low", "Threshold_High", "Shoulder", "Threshold_Mode",
    "Step_Below_pct", "Step_Above_pct", "Status_Below", "Status_Above",
)
...
def _noise_limited_counts(frames: Sequence[FrameResult]) -> tuple:
    """Frames whose Below / either / Above side is a lower bound, in row order."""
    below = sum(f.status_below == "noise-limited" for f in frames)
    above = sum(f.status_above == "noise-limited" for f in frames)
    either = sum(f.noise_limited for f in frames)
    return (below, either, above)
...
def frame_class_stats(frames: Sequence[FrameResult]) -> List[OpticalClassStat]:
    ...
    limited = _noise_limited_counts(frames)
    ...
    return [
        OpticalClassStat(
            ...
            n_frames=len(frames),
            n_noise_limited=n_limited,
        )
        for label, cov, con, n_limited in zip(labels, coverage, contrast, limited)
    ]
...
def frame_class_rows(frames: Sequence[FrameResult]) -> List[list]:
    ...
    return [
        [
            entry.label, position, entry.n_frames, entry.n_noise_limited,
...
def frame_point_rows(frames: Sequence[FrameResult]) -> List[list]:
    ...
    for frame in sorted(frames, key=lambda f: f.point):
        ...
        rows.append([
            ...
            frame.threshold_mode, frame.step_below, frame.step_above,
            frame.status_below, frame.status_above,
        ])
    return rows
