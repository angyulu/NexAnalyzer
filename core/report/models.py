...
class OpticalClassStat(NamedTuple):
    ...
    ``n_noise_limited`` counts frames whose cut for this class was held at the
    noise floor, making their coverage a lower bound rather than a
    measurement. Reported beside the mean rather than folded into it, so the
    average is read with that in mind.
    """
